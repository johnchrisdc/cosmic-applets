...
impl Window {
    ...
    fn vertical_layout(&self) -> Element<'_, Message> {
        ...
        let elements: Vec<Element<'_, Message>> = if let Some(strftime) = self.maybe_strftime() {
            ...
            if self.isWordClock || self.is_word_clock {
                let formatted_time = time_to_words(
                    self.now.hour() as u32,
                    self.now.minute() as u32,
    ...
    fn horizontal_layout(&self) -> Element<'_, Message> {
        let formatted_date = if let Some(strftime) = self.maybe_strftime() {
            strftime
        } else if self.isWordClock || self.is_word_clock {
            let formatted_time = time_to_words(
                self.now.hour() as u32,
                self.now.minute() as u32,
                self.config.military_time,
            ...
            if self.config.show_date_in_top_panel {
                ...
                let prefs = DateTimeFormatterPreferences::from(self.locale.clone());
                let date_str = if self.config.show_weekday {
                    DateTimeFormatter::try_new(prefs, fieldsets::MDE::medium())
                        .unwrap()
                        .format(&datetime)
                        .to_string()
                } else {
                    DateTimeFormatter::try_new(prefs, fieldsets::MD::medium())
                        .unwrap()
                        .format(&datetime)
                        .to_string()
                };
                format!("{date_str} {formatted_time}")
            } else {
                formatted_time
            }
        } else {
...
impl cosmic::Application for Window {
    ...
    type Flags = ();
    const APP_ID: &str = "com.system76.CosmicAppletPotatoTime";
    ...
    fn update(&mut self, message: Self::Message) -> app::Task<Self::Message> {
        match message {
            ...
            Message::ConfigChanged(c) => {
                ...
                self.isWordClock = c.is_word_clock;
                self.is_word_clock = c.is_word_clock;
                self.config = c;
                Task::none()
            }
...
fn number_below_60_to_words(n: u32) -> String {
    match n {
        0..=20 | 30 | 40 | 50 => number_to_words(n).to_string(),
...
fn hour_to_words(hour: u32, military_time: bool) -> String {
    if military_time {
        number_below_60_to_words(hour)
    } else {
        let h12 = match hour % 12 {
            0 => 12,
            h => h,
        };
        number_to_words(h12).to_string()
    }
}
...
fn minute_to_words(minute: u32) -> String {
    if minute == 0 {
        ...
        number_below_60_to_words(minute)
    }
}

pub fn time_to_words(
    hour: u32,
    minute: u32,
    military_time: bool,
    show_seconds: bool,
    ...
    let h = hour_to_words(hour, military_time);
    let m = minute_to_words(minute);
    if show_seconds && second > 0 {
        let s = number_below_60_to_words(second);
        format!("{h} {m} {s}")
    } else {
        format!("{h} {m}")
    }
...
mod tests {
    ...
    fn test_time_to_words() {
        assert_eq!(time_to_words(12, 9, false, false, 0), "Twelve Nine");
        assert_eq!(time_to_words(12, 0, false, false, 0), "Twelve o'clock");
        assert_eq!(time_to_words(0, 0, false, false, 0), "Twelve o'clock");
        assert_eq!(time_to_words(1, 5, false, false, 0), "One Five");
        assert_eq!(time_to_words(14, 45, false, false, 0), "Two Forty Five");
    }
}
